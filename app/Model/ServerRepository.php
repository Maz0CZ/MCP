...
final class ServerRepository
{
    ...
    public function create(int $userId, int $packageId, int $port, string $directory, string $status, string $screenName): int
    {
        $stmt = $this->pdo->prepare('INSERT INTO servers (user_id, package_id, port, directory, status, screen_name) VALUES (:user_id, :package_id, :port, :directory, :status, :screen_name)');
        $stmt->execute([
            'user_id' => $userId,
    ...
    {
        $stmt = $this->pdo->query('SELECT servers.*, users.email AS user_email, packages.name AS package_name FROM servers JOIN users ON users.id = servers.user_id JOIN packages ON packages.id = servers.package_id ORDER BY servers.id ASC');
